...
def determine_packaging(env, schedule):
	...
	for n in demand['doc_num']:
		...
		if zfin_inventory_temp[zfin_data[order_data['zfin'].values[0]][-1]] >= order_data['order_qty'].values[0]:
			zfin_inventory_temp[zfin_data[order_data['zfin'].values[0]][-1]] -= order_data['order_qty'].values[0]
		# Assign capacity
		else:
			# Get matching indices for product availability
			idxs = schedule.loc[
				(schedule['qty_remaining'] >= qty) &
				(schedule['gmid'] == order_data['gmid'].values[0])
			].index
